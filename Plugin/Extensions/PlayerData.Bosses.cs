...
public static partial class PlayerDataExtensions
{
	...
	public static bool HasKilledBoss(this PlayerData data, Boss boss)
	{
		return boss switch
		{
			...
			Boss.MossMothers => data.HasObtainedJournalEntry(Boss.MossMothers),
			...
			Boss.SkullTyrant2 => data.HasObtainedJournalEntry(Boss.SkullTyrant2),
			...
			Boss.Garmond => data.HasObtainedJournalEntry(Boss.Garmond),
			Boss.GrandMotherSilk => data.HasCompletedEnding(Ending.WeaverQueen)
				|| data.HasCompletedEnding(Ending.SnaredSilk)
			...
			Boss.Pinstress => data.HasObtainedJournalEntry(Boss.Pinstress),
